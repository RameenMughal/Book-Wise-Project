...
void removebook()
{
   ...
   {sscanf(line, "\t\t| %5d", &b.id);
    if (b.id == delid)
        {printf("\n\t\tBook with ID %d removed successfully.", delid);
        found = 1;
        break;  }
    else
        {fprintf(ft, "%s", line);}}
...
void issuebook()
{
    ...
    // Open the Books List and check the required book ID
    fp = fopen("Books List.txt", "r");
    ...
    if(b.id!=idchoice)
       ...
       {// Input Values
        ...
        fprintf(fp, "|%8d  |  %-31s|  %-31s|  %-10d|  %-10d|  %10s|\n", s.id,s.studentname,
		s.studentclass,s.rollnumber, b.id, s.date);
        fclose(fp);
